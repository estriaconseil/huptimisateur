-- Migration delta : ajout de la table salespeople et correction des références.
-- Touche UNIQUEMENT les tables du module ventes (créées aujourd'hui, sans données réelles).
-- Les tables clients, jobs, schedules, teams, etc. ne sont PAS affectées.

-- ── 1. Créer la table salespeople (nouvelle) ─────────────────────────────────
...
-- ── 2. Insérer les vendeurs de départ ────────────────────────────────────────
...
-- ── 3. Reconstruire les tables ventes avec la bonne référence ─────────────────
-- Ces tables ont été créées aujourd'hui et sont vides.
-- On les recrée pour pointer vers salespeople au lieu de profiles.

DROP TABLE IF EXISTS public.quote_units        CASCADE;
DROP TABLE IF EXISTS public.quotes             CASCADE;
DROP TABLE IF EXISTS public.sales_appointments CASCADE;
DROP TABLE IF EXISTS public.salesperson_config CASCADE;  -- remplacée par salespeople
